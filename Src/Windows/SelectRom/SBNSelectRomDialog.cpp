...
namespace sbn {
	...
	CWidget::LSW_HANDLED CSelectRomDialog::InitDialog() {
		...
		CListBox * plvList = static_cast<CListBox *>(FindChild( CSelectRomDialogLayout::SBN_SFI_LISTBOX ));
	...
	CWidget::LSW_HANDLED CSelectRomDialog::Command( WORD _wCtrlCode, WORD _wId, CWidget * _pwSrc ) {
		switch ( _wCtrlCode ) {
			...
			case EN_CHANGE : {
				CListBox * plvList = static_cast<CListBox *>(FindChild( CSelectRomDialogLayout::SBN_SFI_LISTBOX ));
				CEdit * peEdit = static_cast<CEdit *>(FindChild( CSelectRomDialogLayout::SBN_SFI_EDIT_SEARCH ));
				if ( peEdit && plvList ) {
		...
		switch ( _wId ) {
			case CSelectRomDialogLayout::SBN_SFI_BUTTON_OK : {
				CListBox * plvList = static_cast<CListBox *>(FindChild( CSelectRomDialogLayout::SBN_SFI_LISTBOX ));
				if ( plvList ) {
					if ( plvList->GetCurSel() != LB_ERR ) {
						::EndDialog( Wnd(), plvList->GetCurSelItemData() );
			...
			case CSelectRomDialogLayout::SBN_SFI_BUTTON_CANCEL : {
				return Close();
			}
		}
