...
#include "SBNControllerSetupWindow.h"
...
namespace sbn {
	...
	CControllerSetupWindow::CControllerSetupWindow( const lsw::LSW_WIDGET_LAYOUT &_wlLayout, lsw::CWidget * _pwParent, bool _bCreateWidget, HMENU _hMenu, uint64_t _ui64Data ) :
		...
		m_pioOptions( reinterpret_cast<SBN_CONTROLLER_SETUP_DATA *>(_ui64Data)->pioOptions ),
		m_pmwMainWindow( reinterpret_cast<SBN_CONTROLLER_SETUP_DATA *>(_ui64Data)->pmwMainWindow ),
		m_stPlayerIdx( reinterpret_cast<SBN_CONTROLLER_SETUP_DATA *>(_ui64Data)->stIdx ) {
	}
	...
	CWidget::LSW_HANDLED CControllerSetupWindow::InitDialog() {
		...
		if ( ptTab ) {
			...
			static LPWSTR const lpwstrTabTitles[] = {
				const_cast<LPWSTR>(SBN_LSTR( SBN_STD_INPUT_BUTTONS )),
				const_cast<LPWSTR>(SBN_LSTR( SBN_STD_INPUT_ALTTERNATIVE_BUTTONS_1 )),
				const_cast<LPWSTR>(SBN_LSTR( SBN_STD_INPUT_ALTTERNATIVE_BUTTONS_2 )),
				const_cast<LPWSTR>(SBN_LSTR( SBN_STD_INPUT_ALTTERNATIVE_BUTTONS_3 )),
			};
			for ( size_t I = SBN_ELEMENTS( lpwstrTabTitles ); I--; ) {
	...
	CWidget::LSW_HANDLED CControllerSetupWindow::Command( WORD /*_wCtrlCode*/, WORD _wId, CWidget * /*_pwSrc*/ ) {
		switch ( _wId ) {
			case Layout::SBN_CSW_OK : {
				SaveAndClose();
				return LSW_H_HANDLED;
			}
			case Layout::SBN_CSW_CANCEL : {
				Close();
				return LSW_H_HANDLED;
			}
		}
	...
	LSW_RECT CControllerSetupWindow::ClientRect( const CWidget * _pwChild ) const {
		if ( _pwChild && _pwChild->Id() == CStdControllerPageLayout::SBN_SCPI_MAINWINDOW ) {
			CTab * ptTab = GetTab();
			LSW_RECT rClient = ptTab->ClientRect();
