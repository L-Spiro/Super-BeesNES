...
namespace sbn {
	...
	CWidget::LSW_HANDLED CInputWindow::InitDialog() {
		...
		CTab * ptTab = static_cast<CTab *>(FindChild( Layout::SBN_IWI_TAB ));
		...
		if ( ptTab ) {
			...
			CWidget * pwOkButton = FindChild( Layout::SBN_IWI_OK );
			CWidget * pwCancelButton = FindChild( Layout::SBN_IWI_CANCEL );
			LSW_RECT rTmp;
			rTmp.Zero();
			if ( pwOkButton ) {
				rTmp = pwOkButton->WindowRect();
				rTmp.MoveBy( -rTmp.left, -rTmp.top );
				rTmp.MoveBy( rTab.Width() - rTmp.Width() * 2 - SBN_LEFT_JUST * 2, rTab.Height() + 2 );
				::MoveWindow( pwOkButton->Wnd(), rTmp.left, rTmp.top, rTmp.Width(), rTmp.Height(), FALSE );
			}
			if ( pwCancelButton ) {
				rTmp = pwCancelButton->WindowRect();
				rTmp.MoveBy( -rTmp.left, -rTmp.top );
				rTmp.MoveBy( rTab.Width() - rTmp.Width() - SBN_LEFT_JUST, rTab.Height() + 2 );
	...
	CWidget::LSW_HANDLED CInputWindow::Command( WORD /*_wCtrlCode*/, WORD _wId, CWidget * /*_pwSrc*/ ) {
		switch ( _wId ) {
			case Layout::SBN_IWI_CANCEL : {
				return Close();
			}
			case Layout::SBN_IWI_OK : {
				SaveAndClose();
				return LSW_H_HANDLED;
			}
		}
	...
	const LSW_RECT CInputWindow::VirtualClientRect( const CWidget * pwChild ) const {
		if ( pwChild->Id() == CInputWindowLayout::SBN_IWI_PER_GAME_SETTINGS_PANEL || pwChild->Id() == CInputWindowLayout::SBN_IWI_GLOBAL_SETTINGS_PANEL ) {
			const CTab * ptTab = static_cast<const CTab *>(FindChild( Layout::SBN_IWI_TAB ));
			if ( ptTab ) {
