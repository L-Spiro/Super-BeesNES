...
#include "../../Input/SBNControllerListener.h"
...
namespace sbn {
	...
	public :
		CStdControllerPage( const LSW_WIDGET_LAYOUT &_wlLayout, CWidget * _pwParent, bool _bCreateWidget = true, HMENU _hMenu = NULL, uint64_t _ui64Data = 0 ) :
			...
			m_stPlayerIdx( reinterpret_cast<SBN_CONTROLLER_SETUP_DATA *>(_ui64Data)->stConfigureIdx >> 16 ),
			m_stConfigIdx( reinterpret_cast<SBN_CONTROLLER_SETUP_DATA *>(_ui64Data)->stConfigureIdx & 0xFFFF ) {
		}
		~CStdControllerPage() {
		}
		 ...
		 */
		virtual const wchar_t *						ListeningText() const { return SBN_LSTR( SBN_LISTENING_ ); }
	...
	protected :
		// == Members.
		/** The options object. */
		SBN_INPUT_OPTIONS *							m_pioOptions;
		/** The main window. */
		lsn::CMainWindow *							m_pmwMainWindow;

		/** The primary buttons. */
		SBN_INPUT_EVENT								m_ieMainButtons[8];
		/** The turbo buttons. */
		SBN_INPUT_EVENT								m_ieTurboButtons[8];
