...
namespace sbn {
	 ...
	 **/
	void CUtilities::DeconstructFilePath( const std::u16string &_s16Path, SBN_FILE_PATHS &_fpPaths ) {
	 ...
	 **/
	std::vector<CUtilities::SBN_FILE_PATHS> CUtilities::DeconstructFilePaths( const std::vector<std::u16string> &_s16Paths ) {
		std::vector<CUtilities::SBN_FILE_PATHS> vPaths;
		if ( _s16Paths.size() ) {
	 ...
	 **/
	bool CUtilities::DuplicateFiles( const std::vector<CUtilities::SBN_FILE_PATHS> &_s16Paths, std::vector<size_t> &_vIndices ) {
		_vIndices.clear();
		for ( size_t I = 0; I < _s16Paths.size(); ++I ) {
			bool bFound = false;
